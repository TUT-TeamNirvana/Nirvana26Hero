...
static uint8_t last_custom_r = 0;                // 记忆上一次右自定义键状态
static uint8_t pause_cd = 0;                     // 暂停键冷却,防抖处理
static uint8_t custom_l_cd = 0;                  // 左自定义键冷却,防抖处理
static uint8_t custom_r_cd = 0;                  // 右自定义键冷却,防抖处理

// 摩擦轮与拨弹盘图传专属状态机
static uint8_t vtx_friction_on = 0;  // 右自定义键维护的摩擦轮状态
static uint8_t vtx_shoot_on = 0;     // 拨轮向上维护的开火状态(正转)
static uint8_t last_dial_up = 0;     // 记录上一次拨轮是否处于向上状态
...
static void vtx_to_rc(const uint8_t *vtx_buf)
{
    ...
    uint8_t custom_r  = (vtx_buf[8] & 0x01);
    uint8_t trigger   = (vtx_buf[9] >> 4) & 0x01;

    // 冷却倒计时
    if (pause_cd > 0) pause_cd--;
    if (custom_l_cd > 0) custom_l_cd--;
    if (custom_r_cd > 0) custom_r_cd--;
    ...
    if (custom_l && !last_custom_l && custom_l_cd == 0) {
    ...
    // 【右自定义键】 -> 摩擦轮开关 (映射给键盘左上角的 Q 键，或者直接利用它)
    // 拨轮值提取用于图传专门逻辑
    uint8_t current_dial_up = (rc_ctrl[TEMP].rc.dial > 100);
    uint8_t current_dial_down = (rc_ctrl[TEMP].rc.dial < -100);

    if (current_dial_up && !last_dial_up) {
        vtx_shoot_on = !vtx_shoot_on; // 向上拨动切换正转
    }
    if (current_dial_down) {
        vtx_shoot_on = 0; // 向下时关闭正转切换状态
    }

    if (custom_r && !last_custom_r && custom_r_cd == 0) {
        vtx_friction_on = !vtx_friction_on; // 切换摩擦轮状态
        custom_r_cd = 10;
    }

    last_dial_up = current_dial_up;
    last_pause_btn = pause_btn;
    last_custom_l = custom_l;
    last_custom_r = custom_r;
    ...
    *(uint16_t *)&rc_ctrl[TEMP].key[KEY_PRESS] = (uint16_t)(vtx_buf[17] | (vtx_buf[18] << 8));
    ...
    // 伪造按键/鼠标以适配 MouseKeySet
    if (rc_ctrl[TEMP].key[KEY_PRESS].f) vtx_friction_on = 1;
    if (rc_ctrl[TEMP].key[KEY_PRESS].g) vtx_friction_on = 0;

    if (vtx_friction_on) {
        rc_ctrl[TEMP].key[KEY_PRESS].f = 1;
        rc_ctrl[TEMP].key[KEY_PRESS].g = 0;
    } else {
        rc_ctrl[TEMP].key[KEY_PRESS].f = 0;
        rc_ctrl[TEMP].key[KEY_PRESS].g = 1;
    }

    if (vtx_shoot_on) {
        rc_ctrl[TEMP].mouse.press_l = 1;
    } 
    if (current_dial_down) {
        rc_ctrl[TEMP].key[KEY_PRESS].c = 1;
    }

    rc_ctrl[TEMP].rc.dial = 0; // 清楚图传拨轮值，避免干扰原有逻辑
